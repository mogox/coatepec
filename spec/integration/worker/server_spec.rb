...
RSpec.describe "coatepec-worker executable", type: :integration do
  ...
  it "answers a spec_run command for a Minitest test path through the same worker" do
    ...
    protocol.write(id: 1, command: "spec_run",
                   args: { paths: ["test/models/passing_test.rb:9"], include_passing: true })
    response = protocol.read
  ...
  it "answers a controller command" do
    @pid, protocol = spawn_worker

    protocol.write(id: 1, command: "controller", args: { name: "WidgetsController" })
    response = protocol.read

    expect(response[:ok]).to be(true)
    expect(response[:data][:name]).to eq("WidgetsController")
    expect(response[:data][:controller_path]).to eq("widgets")
    expect(response[:data][:actions]).not_to be_empty
  end

  it "answers with a structured error for an invalid controller name" do
    @pid, protocol = spawn_worker

    protocol.write(id: 1, command: "controller", args: { name: "not_a_constant" })
    response = protocol.read

    expect(response[:ok]).to be(false)
    expect(response[:error][:code]).to eq("invalid_controller_name")
  end
end
